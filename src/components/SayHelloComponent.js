...
export default function SayHelloComponent() {
  ...
  const [output, getOutput] = useState("Hello [UserName]! You look great today!");
  const getResult = async () =>{
    const response = await SayHello(string);
    getOutput(response);
  }
  return (
    <body class="backgroundSayHello">
    <div>
        <div class="grid grid-cols-12">
            ...
            <div class="col-span-8 text-center mt-[3.8125rem] mb-[3.4376rem]">
                <p class="font-[Rubik-Bold] text-[3.125rem] lg:text-[5rem] text-white">Say Hello</p>
            </div>
        </div>
        <div>
        ...
        <div class="flex justify-center px-[1.9375rem]">
            <div
                ...
                <p class="font-[Rubik-Bold] text-[1.875rem] lg:text-[4.375rem] text-center p-5 lg:p-20" id="returnOutput">{output}</p>
            </div>
        </div>

        <div class="flex justify-center">
            <p class="text-[1.875rem] lg:text-[3.125rem] font-[Rubik-Bold] mt-[1.25rem] lg:mt-[5.25rem] text-center">
                What is your name</p>
        </div>
        ...
        <div class="flex justify-center px-[2.125rem] ">
            <input
                ...
                type="text" placeholder="Name" onChange ={(e) => getString(e.target.value)}/>
        </div>

        <div class=" flex justify-center">
            <button type="submit" value="0" onClick={() => getResult()}
                class=" bg-[#00FF75] rounded-[20px] border-black border-[0.3125rem] text-center w-[17.1875rem]
