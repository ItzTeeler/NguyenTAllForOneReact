...
export default function ReverseItStringComponent() {
  ...
  const [output, getOutput] = useState("The word [userinput] reversed [output]");
  const getResult = async () =>{
    const response = await ReverseItStringApi(string);
    getOutput(response);
  }
  return (
<body class="backgroundReverseItString">
    <div>
        <div class="grid grid-cols-12">
            ...
            <div class="col-span-8 text-center mt-[3.8125rem] mb-[3.4376rem]">
                <p class="font-[Rubik-Bold] text-[3.125rem] lg:text-[5rem] text-white">Reverse It (String)</p>
            </div>
        </div>
        <div>
        ...
        <div class="flex justify-center">
            <p class="text-[1.875rem] lg:text-[3.125rem] font-[Rubik-Bold] mt-[1.25rem] lg:mt-[5.25rem]">Enter a word!</p>
        </div>
        ...
        <div class="flex justify-center px-[2.125rem] ">
            <input
                ...
                type="text" placeholder="Input" onChange ={(e) => getString(e.target.value)}/>
        </div>
